...
def add_mm_charges(scf_method, atoms_or_coords, a, charges, radii=None, 
        rcut_ewald=None, rcut_pc=None, rcut_dip=None, unit=None):
    ...
    mm_mol = mm_mole.create_mm_mol(atoms_or_coords, a, charges, radii=radii, 
            rcut_ewald=rcut_ewald, rcut_pc=rcut_pc, rcut_dip=rcut_dip,
            unit=unit)
...
def qmmm_for_scf(scf_method, mm_mol):
    ...
    class QMMM(_QMMM, method_class):
        ...
        def get_hcore(self, mol=None, rcut_pc=None, rcut_dip=None, s1r=None):
            ...
            if rcut_pc >= np.min(r_qm_qm):
                logger.warn(self, 'QM image is within rcut_pc of QM')
            Ls[Ls == np.inf] = 0.0
